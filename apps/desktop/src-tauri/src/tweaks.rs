...
const READ_SCRIPT: &str = "[Console]::OutputEncoding=[Text.Encoding]::UTF8; $adv='HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced'; $per='HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize'; $srch='HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Search'; function getval($p,$n){ try{ (Get-ItemProperty -Path $p -Name $n -ErrorAction Stop).$n }catch{ $null } } $o=[ordered]@{}; $o['dark-theme']=((getval $per 'AppsUseLightTheme') -eq 0); $o['file-ext']=((getval $adv 'HideFileExt') -eq 0); $o['hidden-files']=((getval $adv 'Hidden') -eq 1); $o['taskbar-left']=((getval $adv 'TaskbarAl') -eq 0); $o['hide-widgets']=((getval $adv 'TaskbarDa') -eq 0); $o['hide-chat']=((getval $adv 'TaskbarMn') -eq 0); $o['explorer-thispc']=((getval $adv 'LaunchTo') -eq 1); $o['bing-off']=((getval $srch 'BingSearchEnabled') -eq 0); $o['telemetry-off']=((getval 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection' 'AllowTelemetry') -eq 0); $o['consumer-off']=((getval 'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\CloudContent' 'DisableWindowsConsumerFeatures') -eq 1); $o['power-high']=((powercfg /getactivescheme) -match '8c5e7fda'); $o['hibernate-off']=((getval 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Power' 'HibernateEnabled') -eq 0); [pscustomobject]$o | ConvertTo-Json -Compress";

// Applies one HKCU tweak; key + on passed via env so the script stays fixed.
const USER_APPLY_SCRIPT: &str = "$k=$env:FORJA_TWEAK_KEY; $on=$env:FORJA_TWEAK_ON -eq '1'; $adv='HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced'; $per='HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize'; $srch='HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Search'; function setdw($p,$n,$v){ if(-not(Test-Path $p)){New-Item -Path $p -Force | Out-Null}; Set-ItemProperty -Path $p -Name $n -Type DWord -Value $v } switch($k){ 'dark-theme'{ $v=[int](-not $on); setdw $per 'AppsUseLightTheme' $v; setdw $per 'SystemUsesLightTheme' $v } 'file-ext'{ setdw $adv 'HideFileExt' ([int](-not $on)) } 'hidden-files'{ setdw $adv 'Hidden' $(if($on){1}else{2}) } 'taskbar-left'{ setdw $adv 'TaskbarAl' $(if($on){0}else{1}) } 'hide-widgets'{ setdw $adv 'TaskbarDa' $(if($on){0}else{1}) } 'hide-chat'{ setdw $adv 'TaskbarMn' $(if($on){0}else{1}) } 'explorer-thispc'{ setdw $adv 'LaunchTo' $(if($on){1}else{2}) } 'bing-off'{ setdw $srch 'BingSearchEnabled' $(if($on){0}else{1}) } }";
